"""
AC Optimal Power Flow Batch Study
Runs comprehensive benchmark studies on AC OPF problems with various solution strategies.
"""
...
# ---------------------------------------------------------------------------
# Dual-mode constants for extended dual decomposition experiments
# ---------------------------------------------------------------------------
# "none"              -> no dual info supplied (IPOPT initializes from scratch)
# "full"              -> constraint duals (lambda*) + block-matched bound mults (zL/zU for supplied primal blocks)
# "constraint_only"   -> constraint duals (lambda*) only; zL/zU left to IPOPT
# "bounds_only"       -> block-matched bound mults (zL/zU) only; constraint duals left to IPOPT
# "all_bounds_only"   -> ALL bound mults (all 4 blocks) only; no constraint duals
# "full_all_bounds"   -> constraint duals + ALL bound mults (all 4 blocks, not just supplied primal blocks)
DUAL_MODES = ("none", "full", "constraint_only", "bounds_only", "all_bounds_only", "full_all_bounds")

# Extended dual families: each is (family_name, dual_mode).
# These run over all 15 non-empty primal-block subsets, like the existing oracle families.
...
# Dual-only experiments: empty primal set + various dual configurations.
# These are single runs (one per dual mode) to test whether dual info alone helps.
DUAL_ONLY_VARIANTS = [
...
def build_warmstart_package(full_ws, blocks, dual_mode="none"):
    """Build an initialization package with configurable dual content.

    Parameters
    ----------
    full_ws : dict
        Full warm-start data from a converged solve (primal, dual, zL, zU).
    blocks : tuple of str
        Primal blocks to include (subset of BLOCKS). May be empty for
        dual-only experiments.
    dual_mode : str
        One of:
        - "none"             : no dual info (IPOPT initializes from scratch)
...
def solve_one_ac(ctx, family, blocks, init_package, case_output_dir, baseline_ws=None,
    ...
    if ok:
        ...
        print(f"⚠️ {run_name}: {term_str} | iter={solver_iterations} | solve={solve_time:.3f}s")
...
def run_case_study(case_file, output_dir, tee_baseline=False, tee_others=False, max_iter=300, tol=1e-6,
    ...
    # ---- Core oracle families (primal-only and primal-plus-dual) ----
    ...
    # ---- Extended dual decomposition families ----
    if include_extended_dual_decomposition:
        # Three new families over all 15 primal-block subsets
        ...
        # Dual-only experiments: empty primal, various dual configurations
